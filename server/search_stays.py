    ...
    for hotel_name, data in hotel_data.items():
        ...
        for i in range(len(data) - num_consecutive_days + 1):
            ...
            if all((dates[j+1] - dates[j]).days == 1 for j in range(len(dates)-1)):
                ...
                # if standard_rate <= max_points_budget:
                #     print(f"Standard rate: {str(standard_rate)}")
                #     print(f"Max points budget: {str(max_points_budget)}")
                #     print(f"Rate filter: {rate_filter}")
                #     print(standard_rate <= max_points_budget and standard_rate > 0 and rate_filter != 'Premium')
                #     print(premium_rate <= max_points_budget and premium_rate > 0 and rate_filter != 'Standard')
...
consecutive_stays = search_by_consecutive_nights('2023-08-01',
        '2023-08-31',
        7,
        hotel_name_text='',
        hotel_city='',
